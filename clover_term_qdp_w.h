...
class QDPCloverTermT
{
 public:
  ...
  typedef OLattice<PScalar<PScalar<RScalar<typename WordType<T>::Type_t>>>>
      LatticeREAL;
                                  ...
                                  const QDPCloverTermT<T, U> &from)
{
  ...
  {
    RealT ff =
        where(param.anisoParam.anisoP, Real(1) / param.anisoParam.xi_0, Real(1));
  ...
  RealT diag_mass;
  {
    RealT ff = where(param.anisoParam.anisoP,
                     param.anisoParam.nu / param.anisoParam.xi_0,
                     Real(1));
 ...
 *
 *  chi = (1 - (ClovCoeff/u0^3) * kappa/4 * sum_mu sum_nu F(mu,nu)*sigma(mu,nu))
 * psi
 ...
 *     == psi - kappa * chi
 *
 *  We have absorbed ClovCoeff/u0^3 into kappa. A u0 was previously absorbed
 into kappa
...
void QDPCloverTermT<T, U>::makeClov(const multi1d<U> &f, const RealT &diag_mass)
{
  ...
  QDPCloverEnv::QDPCloverMakeClovArg<U> arg = {
      diag_mass, f0, f1, f2, f3, f4, f5, tri};
  ...
  for (int ssite = lo; ssite < hi; ++ssite) {
    ...
    for (int block = 0; block < 2; block++) {
      ...
      for (int j = 0; j < N; ++j) {
        ...
        // A( j+1 : n, j) = ( A(j+1:n, j) - A(j+1:n, 1:j-1)v(1:k-1) ) /
        // v(j)
        //
...
void QDPCloverTermT<T, U>::applySite(T &chi, const T &psi, int isign, int site) const
{
  ...
  cchi[6] =
      tri[site].diag[1][0] * ppsi[6] + conj(tri[site].offd[1][0]) * ppsi[7] +
  ...
  cchi[7] =
      tri[site].diag[1][1] * ppsi[7] + tri[site].offd[1][0] * ppsi[6] +

...
// double precision versions of previous methods
// TODO: Avoid repetition (with preprocessor magic?)
void to_qc_double(double * output,
    double ** qphix_ferm,
    int nx, int nt) {
  QPHIX_FERM(psi_s) = (double (**)[3][4][2][OUTER_SOALEN]) qphix_ferm;
  int nvecs = nx / (2 * OUTER_SOALEN);
...
void from_qc_double(double * input,
    double ** qphix_ferm,
    int nx, int nt) {
  QPHIX_FERM(psi_s) = (double (**)[3][4][2][OUTER_SOALEN]) qphix_ferm;
  int nvecs = nx / (2 * OUTER_SOALEN);
